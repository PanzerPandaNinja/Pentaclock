jQuery(document).ready(function($) {
    ...
    function light(column, row) {
		document.getElementById("layer"+column+row).style.fill = 'blue'
    }
    ...
    function dim(column, row) {
        document.getElementById("layer"+column+row).style.fill = 'red'
    }
    ...
    function displayNumber(number, position) {
        ...
        displayDigit(parseInt(str[str.length - 1]), position );
        if (str.length > 1) {
            displayDigit(parseInt(str[str.length - 2]), position + 1);
        }
    }
